...
const init = (async function () {
  ...
  for (var y = 1; y < 54; y++) {
    ...
    await page.goto(
      `http://www.tbca.net.br/base-dados/composicao_alimentos.php?pagina=${y}`
    );
    //debug
    ...
    const content = await page.evaluate(() => {
      ...
      const itensPage =
        document.documentElement.querySelectorAll("tbody tr").length;
      const data = {};
      const codsPage = [];
      ...
      for (var x = 0; x < itensPage; x++) {
        ...
        for (var i = 0; i < 6; i++) {
          ...
          val.push(
            document.documentElement.querySelectorAll("thead tr th").item(i)
              .innerText
          );
    ...
    for (var index = 0; index < contentRef.length; index++) {
      ...
      await page.goto(
        `http://www.tbca.net.br/base-dados/int_composicao_alimentos.php?cod_produto=${codigo}`
      );
